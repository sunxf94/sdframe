...
class Medoo
{
    ...
    public function __construct($options = null)
    {
        ...
        if (in_array($this->type, ['mariadb', 'mysql', 'pgsql', 'sybase', 'mssql']) && isset($options['charset'])) {
    ...
    public function logging($enable = true) {
        $this->logging = $enable ? true : false;
        return $this;
    }
    ...
    {
        $quoto = $this->type == 'mysql' ? '`' : '"';
        if (strpos($string, '.') !== false) {
            return $quoto . $this->prefix . str_replace('.', $quoto.'.'.$quoto, $string) . $quoto;
        }
        return $quoto . $string . $quoto;
    }
    ...
    public function insert($table, $datas, $lastId = true)
    {
        ...
        foreach ($datas as $data) {
            ...
            foreach ($columns as $key) {
                ...
                if (!isset($data[$key])) {
                    ...
                    case 'array':
                        $map[$map_key] = [
                            strpos($key, '[JSON]') === strlen($key) - 6 ? json_encode($value, JSON_UNESCAPED_UNICODE) : serialize($value),
                            \PDO::PARAM_STR
                        ];
                        break;
                    case 'object':
        ...
        $res = $this->exec('INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ', $fields) . ') VALUES ' . implode(', ', $stack), $map);
        if ($lastId) {
            $res = $this->id();
        }
        return $res;
    ...
    public function update($table, $data, $where = null, $rowCount = true)
    {
        $fields = [];
        $map = [];
        foreach ($data as $key => $value) {
        ...
        $statement = $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause($where, $map), $map);
        return $rowCount && $statement ? $statement->rowCount() : $statement;
    }

    public function delete($table, $where, $rawCount = true)
    {
        $map = [];
        $statement = $this->exec('DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where, $map), $map);
        return $rawCount && $statement ? $statement->rowCount() : $statement;
    }
