...
class Logger
{
    ...
    private static function wLog($message, $level = self::INFO, $type=self::FILE, $destination='', $extra='') {
        ...
        if($destination==''){
            $_destination=SDF_HOME_PATH.DIRECTORY_SEPARATOR.self::LOG_DIR.DIRECTORY_SEPARATOR.'.'.self::getLogFileName($message,$level);
        }else{
    ...
    private static function mkLogDir(){
        if (!is_dir(SDF_HOME_PATH.DIRECTORY_SEPARATOR.self::LOG_DIR)) {
            if(!mkdir(SDF_HOME_PATH.DIRECTORY_SEPARATOR.self::LOG_DIR)) {
                return false;
            }
        }
        return true;
